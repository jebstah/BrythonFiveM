...
function es_CarShop.firstRunCheck()
	PerformHttpRequest("http://" .. ip .. ":" .. port .. "/vehicles", function(err, rText, headers)
		if err == 0 then
...
AddEventHandler("es:playerLoaded", function(identifier, callback)
    PerformHttpRequest("http://" .. ip .. ":" .. port .. "/vehicles", function(err, rText, headers)
		PerformHttpRequest("http://" .. ip .. ":" .. port .. "/vehicles/" .. json.decode(rText).uuids[1], function(err, rText, headers)
			callback(rText, { owner = identifier, model = nil, colour = 0, scolour = "user", plate = 0, wheels = 0, windows = 0, platetype = 0, exhausts = 0, grills = 0, spoiler = 0 })
		end, "PUT", json.encode({ owner = identifier, model = 0, colour = 0, scolour = "user", plate = 0, wheels = 0, windows = 0, platetype = 0, exhausts = 0, grills = 0, spoiler = 0 }), {["Content-Type"] = 'application/json', Authorization = "Basic " .. auth})
